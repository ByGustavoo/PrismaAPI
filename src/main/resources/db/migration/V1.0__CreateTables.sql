...
CREATE TABLE IF NOT EXISTS cartoes (
    ...
    CONSTRAINT cartoes_situacao_check CHECK (situacao IN ('ATIVO', 'INATIVO')),
    CONSTRAINT cartoes_ultimos_digitos_check CHECK (ultimos_digitos IS NULL OR ultimos_digitos ~ '^[0-9]{4}$'),
    CONSTRAINT cartoes_dia_fechamento_check CHECK (dia_fechamento IS NULL OR dia_fechamento BETWEEN 1 AND 31),
    CONSTRAINT cartoes_dia_vencimento_check CHECK (dia_vencimento IS NULL OR dia_vencimento BETWEEN 1 AND 31),
...
CREATE TABLE IF NOT EXISTS lancamentos (
    ...
    CONSTRAINT lancamentos_situacao_check CHECK (situacao IN ('PAGO', 'PENDENTE', 'AGENDADO')),
    CONSTRAINT lancamentos_forma_check CHECK (forma IN ('CONTA', 'CARTAO_CREDITO', 'PIX', 'DINHEIRO')),
    CONSTRAINT lancamentos_origem_unica_check CHECK (num_nonnulls(id_conta, id_cartao) = 1),
...
CREATE TABLE IF NOT EXISTS despesas_recorrentes (
    ...
    CONSTRAINT despesas_recorrentes_situacao_check CHECK (situacao IN ('ATIVO', 'PAUSADO')),
    CONSTRAINT despesas_recorrentes_origem_unica_check CHECK (num_nonnulls(id_conta, id_cartao) = 1)
);
...
CREATE TABLE IF NOT EXISTS metas (
    ...
    data_criacao TIMESTAMPTZ NOT NULL DEFAULT now(),
    data_atualizacao TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT metas_nome_tamanho_minimo CHECK (char_length(trim(nome)) >= 2),
    CONSTRAINT metas_situacao_check CHECK (situacao IN ('ACOMPANHANDO', 'COMPRADA', 'CANCELADA')),
    CONSTRAINT metas_url_check CHECK (url IS NULL OR url ~* '^https?://'),
